...
<!-- 添加代金券 -->
    <div>
        ...
        <div class="minimum">
            <div class="minimum-set" :class="{avcite:curIndex===0}" @click="changeBar(0)">设置最低消费</div>
            <div class="minimum-set" :class="{avcite:curIndex===1}" @click="changeBar(1)">不设最低消费</div>
        </div>
        <!-- 设置最低消费 -->
        <div class="set-add" v-if="curIndex===0">
            <div class="add-fill">
                <div class="fill-warp">
                    <div>减免金额</div>
                    <div class="fill-nums">
                        <input
                            v-model="setlist.mitigate"
                            pattern="[0-9]*"
                            type="number"
                            oninput="if(value.length>6)value=value.slice(0,8)"
                            placeholder="请输入减免金额" />
                        <div class="nums-name">元</div>
                    </div>
                </div>
            </div>
            <div class="add-fill">
                <div class="fill-warp">
                    ...
                    <div class="fill-nums">
                        <input
                            v-model="setlist.limit"
                            pattern="[0-9]*"
                            type="number"
                            oninput="if(value.length>6)value=value.slice(0,8)"
                            placeholder="请输入金额" />
                        <div class="nums-name">元有效</div>
                    </div>
                </div>
            </div>
            <div class="add-fill">
        ...
        <!-- 不设最低消费 -->
        <div class="set-add" v-if="curIndex===1">
            <div class="add-fill">
                <div class="fill-warp">
                    <div>减免金额</div>
                    <div class="fill-nums">
...
export default {
    ...
    data() {
        return {
            ...
            value1:'到店送精美礼品一份到店送精美礼品一 份到店送精美礼品一份到店送精美礼品 一份',
            setlist:{
                mitigate:'',
                limit:'',
                condition:''
